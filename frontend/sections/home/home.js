...
function home_js() {
  ...
  async function heroPageBanner() {
    ...
    try {
      ...
      landing_page.style.backgroundImage = `url(${data[randomIndex].images.jpg.large_image_url})`;
  ...
  function get_search_query() {
    ...
    search_bar.addEventListener("input", () => {
      ...
      const select = document.querySelector(".search_category_selection");
  ...
  function search_function(query, type) {
    ...
    async function search_individual(type, query) {
      ...
      try {
        const { send_data, has_page } = await search_fetch(
          type,
          query,
          page[type],
        );
    ...
    async function search_fetch(type, query, page_number) {
      try {
        ...
        console.log(
          `error while fetching data from fetching type content: ${error}`,
        );
      }
    }
  }
  ...
  function card_ui(content) {
    ...
    if (content.images) {
      ...
      srcUrl = `https://image.tmdb.org/t/p/original${content.poster_path}`;
    } else {

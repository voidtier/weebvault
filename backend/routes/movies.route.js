...
router.get("/genre", authentify, async function (req, res) {
  ...
  try {
    ...
    console.log(
      `error while fetching data from movies_route_genre_list server side : ${error}`,
    );
...
router.get(
  ...
  async function (req, res) {
    try {
      ...
      console.log(
        `error while fetching data from movies_route_genre server side : ${error}`,
      );
      res.status(500).json({ error: "Internal Server Error" });
    }
  },
...
router.get("/:query/:page_number", authentify, async function (req, res) {
  try {
    ...
    console.log(
      `error while fetching data from movie_route_search server side : ${error}`,
    );
    res.status(500).json({ error: "Internal Server Error" });
  }
});

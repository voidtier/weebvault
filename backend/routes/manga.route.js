...
router.get("/genre", authentify, async function (req, res) {
  ...
  try {
    ...
    console.log(`error while fetching data from manga_route_genre_list server side : ${error}`);
...
router.get("/genre/:genre_id/:page_number", authentify, async function (req, res) {
  ...
  try {
    ...
    console.log(`error while fetching data from manga_route_genre server side : ${error}`);
    res.status(500).json({ error: "Internal Server Error" });
  }
})
...
router.get("/:query/:page_number", authentify, async function (req, res) {
  ...
  try {
    ...
    console.log(`error while fetching data from manga_route_search server side : ${error}`);
    res.status(500).json({ error: "Internal Server Error" });
  }
})
